  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 animate-pulse">
      <div className="space-y-2">
        <div className="h-8 w-64 rounded-xl bg-surface-offset" />
        <div className="h-4 w-40 rounded-lg bg-surface-offset" />
      </div>
      ...
      <div className="space-y-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-14 rounded-xl bg-surface-offset" />
        ))}
      </div>
    </div>
  );
