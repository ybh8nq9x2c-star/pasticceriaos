...
export default async function MarketplaceSuppliersPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto space-y-5 sm:space-y-6">
      <div>
        <h1 className="font-playfair text-2xl sm:text-3xl font-bold mb-1">Fornitori collegati</h1>
        <p className="text-sm text-[#6B7280]">Grossisti a cui puoi inviare ordini.</p>
      </div>

      <ConnectSupplierForm />

      <div className="bg-white rounded-2xl border border-[#E5DDD0] divide-y divide-[#F0EAE0]">
        {suppliers.length === 0 && <p className="p-6 text-center text-[#6B7280]">Nessun fornitore collegato.</p>}
        {suppliers.map((s) => (
          <div key={s.connectionId} className="flex items-center justify-between gap-3 px-4 sm:px-5 py-4">
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#6B7280]">Collegato il {new Date(s.connectedAt).toLocaleDateString('it-IT')}</p>
            </div>
            <Link href={`/marketplace/orders/new?connection=${s.connectionId}`}
              className="shrink-0 px-3 py-2 rounded-lg bg-[#1A2B4A] text-white text-xs font-semibold hover:bg-[#243660] min-h-[40px] inline-flex items-center">
              + Ordine
            </Link>
          </div>
        ))}
      ...
      <Link href="/marketplace/orders" className="inline-block text-sm text-[#C9962A] font-semibold hover:underline">
        Vedi i tuoi ordini →
      </Link>
    </div>
  );
