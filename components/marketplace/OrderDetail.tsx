    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto space-y-5 sm:space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs text-ink-muted uppercase">{counterpartyLabel}</p>
          <h1 className="text-xl sm:text-2xl font-bold truncate">{order.counterpartyName}</h1>
          <p className="text-sm text-ink-muted">Ordine #{order.id.slice(0, 8)}</p>
        </div>
      ...
      <div className="md:hidden space-y-2">
        {order.lines.map((l) => (
          <div key={l.id} className="bg-surface-2 rounded-xl border border-border p-3">
            <div className="flex items-start justify-between gap-3">
              <span className="font-medium leading-tight">{l.name}{l.sku ? <span className="text-ink-muted"> · {l.sku}</span> : null}</span>
              <span className="shrink-0 font-mono font-semibold">{l.unitPrice != null ? `${formatCurrency(l.quantity * l.unitPrice)} €` : '—'}</span>
            </div>
            <div className="mt-1 text-xs text-ink-muted">{l.quantity} {l.unit}{l.unitPrice != null ? ` × ${formatCurrency(l.unitPrice)} €` : ''}</div>
          </div>
        ))}
          ...
          <tbody>
            {order.lines.map((l) => (
              <tr key={l.id} className="border-t border-divider">
                <td className="px-4 py-3">{l.name}{l.sku ? <span className="text-ink-muted"> · {l.sku}</span> : null}</td>
      ...
      {order.notes && (
        <div className="bg-surface-2 rounded-2xl border border-border p-4 text-sm">
          <p className="text-xs text-ink-muted uppercase mb-1">Note</p>{order.notes}
        </div>
      )}
        ...
        <ol className="space-y-2 text-sm">
          {order.history.map((h) => (
            <li key={h.id} className="flex justify-between gap-3">
              ...
              <span className="text-ink-muted text-right shrink-0">{new Date(h.createdAt).toLocaleString('it-IT')}</span>
            </li>
          ))}
        </ol>
      </div>
