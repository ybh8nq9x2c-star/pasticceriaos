...
const field = 'rounded-xl border border-[#E5DDD0] px-3 py-2.5 text-base sm:text-sm min-h-[44px]';
  ...
  return (
    <div className="space-y-6">
      <form action={addAction} className="bg-white rounded-2xl border border-[#E5DDD0] p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3 items-end">
        ...
        <button type="submit" className="px-4 py-2.5 rounded-xl bg-[#14B8A6] text-white text-sm font-semibold hover:bg-teal-600 min-h-[44px]">Aggiungi</button>
        {addState.status === 'error' && <p className="col-span-full text-sm text-[#C0392B]">{addState.error}</p>}
      </form>

      {/* Mobile: card stack */}
      <ul className="md:hidden space-y-3">
        {items.filter((i) => i.isActive).length === 0 && (
          <li className="bg-white rounded-2xl border border-[#E5DDD0] p-6 text-center text-[#6B7280]">Nessun prodotto. Aggiungi il primo.</li>
        )}
        {items.filter((i) => i.isActive).map((i) => (
          <li key={i.id} className="bg-white rounded-2xl border border-[#E5DDD0] p-4 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="font-medium truncate">{i.name}{i.sku ? <span className="text-[#6B7280]"> · {i.sku}</span> : null}</p>
              <p className="text-xs text-[#6B7280] mt-0.5">{i.unitPrice != null ? `${i.unitPrice} € / ${i.unit}` : `— / ${i.unit}`}</p>
            </div>
            <form action={delAction} className="shrink-0"><input type="hidden" name="id" value={i.id} /><button className="text-[#C0392B] text-xs font-semibold hover:underline min-h-[36px] px-1">Disattiva</button></form>
          </li>
        ))}
      </ul>
      ...
      <div className="hidden md:block bg-white rounded-2xl border border-[#E5DDD0] overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-[#FAF7F2] text-[#6B7280] text-xs uppercase"><tr><th className="text-left px-4 py-3">Prodotto</th><th className="text-left px-4 py-3">Unità</th><th className="text-right px-4 py-3">€/u</th><th className="px-4 py-3"></th></tr></thead>
          <tbody>
            {items.length === 0 && <tr><td colSpan={4} className="px-4 py-6 text-center text-[#6B7280]">Nessun prodotto. Aggiungi il primo.</td></tr>}
            {items.filter((i) => i.isActive).map((i) => (
              <tr key={i.id} className="border-t border-[#F0EAE0]">
                ...
                <td className="px-4 py-3 text-right">
                  <form action={delAction}><input type="hidden" name="id" value={i.id} /><button className="text-[#C0392B] text-xs font-semibold hover:underline">Disattiva</button></form>
                </td>
              </tr>
            ))}
          </tbody>
